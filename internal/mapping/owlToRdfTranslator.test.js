...
describe("OwlToRdfTranslator internal mapping contract", () => {
  it("keeps the completed structural OWL-to-RDF mapper constructible internally", () => {
    ...
    expect(typeof OwlToRdfTranslator).toBe("function");
  });
  ...
  it("places every generated quad in the caller-selected RDF dataset graph", () => {
    ...
    const graph = rdfDataFactory.namedNode("urn:test:graph");
  ...
  it("keeps one source anonymous individual stable and distinct from generated nodes", () => {
    ...
    const sameAs = [
      ...dataset.match(
        null,
        rdfDataFactory.namedNode("http://www.w3.org/2002/07/owl#sameAs"),
        null,
      ),
    ][0];
    const assertion = [
      ...dataset.match(null, rdfDataFactory.namedNode("urn:test:p"), null),
    ][0];
    const ontologyHeader = [
      ...dataset.match(
        ...
        rdfDataFactory.namedNode("http://www.w3.org/2002/07/owl#Ontology"),
      ),
    ][0].subject;
