...
or `UnrepresentableOntologyError` type.
...
`OWLOntologyStorageError`, not a new public subclass.
...
- retroactively redesign public `0.1.0` bindings that are unrelated to the two
  identified defects;
- publish a package, assign a dist-tag, or change Universal Ontology; or
...
- a `phase21` checkpoint containing the current generated registry SHA-256 and
  `status: "COMPLETE"` only after all Phase 21 gates pass.
...
### Task 6: Complete the Phase 21 checkpoint and hand off to Phase 22
...
1. Run the complete source test suite, lint, formatting, generated-surface,
   package-boundary, browser, and package-lint gates applicable to the accepted
   post-`0.1.0` repository. Do not suppress a pre-existing failure without its
   ordinary disposition process.
...
5. Request authorization for the exact Phase 21 commit. After an approved signed
...
6. Do not tag or publish. Phase 22 may begin only from a branch on which both the
...
record, capability rows, generated registry, forbidden-export assertions, and
Git ancestry before changing manager state. Its storage task may add
